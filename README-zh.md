...
5. 它决定写哪种文档并写出来：小活写 **DoD**（definition of done，"什么算做完"，
   `~/.dsh/crew/jobs/<job-slug>/dod.md`），真正的产品写 **PRD**（product requirements document，
   产品需求文档，`docs/design/prd.md`）。它会说明选了哪种，一个词就能换。
   **开工前必须你确认。**
...
6. 它先把你给的作业名转成一个短 slug——只用小写字母、数字和 `-`，别的都不要——把
   转好的 slug 告诉你，再用它创建 `crew/<job-slug>` 分支。然后，如果是 PRD 类的作业，
   它会启动 `crew_architect`，产出高层设计、决策记录（ADR）和任务拆分。
...
16. **修 bug 时可能会来问你，而且每一次选择都会被写下来。** 这件事可能在第 8 步里
    的任何时候发生。工程师修一个 bug——QA 报的缺陷、代码评审的阻塞发现，或者它自己
    撞见的 bug——会先找出至少两个真的可行的办法。如果这几个办法只是写法不同，它自己
    ...
    一个去处：一条 **ADR**——放在 `docs/decisions/adr/` 的决策记录。PRD 类的作业可以
...
下面，每个目录的名字就说清它装的是什么：PRD 和设计在 `docs/design/`
（每条模块边界一个契约文件，在 `docs/design/api/`）、决策记录和变更请求在
