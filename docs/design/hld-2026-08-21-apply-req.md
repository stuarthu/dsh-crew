...
| 会红的用例 | 谁弄红的 | 为什么 | 承载在哪 |
| --- | --- | --- | --- |
| **`docs/qa/T-56/case-07-no-new-h2.mjs`** | **T-63** | 它断言 `roles/pm.md` 的顶级小节**恰好 13 个**；T-63 加了 `## What you may write`，13 变 14 | **已修好**：`crew-qa-1` 在 T-63 那个提交（`b492ea0`）里改成 14。**这一条本表第一版漏了** |
| `docs/qa/T-52/case-01-principle-numbers-1-to-21.mjs` | **T-68** | 它断言 `principles.md` 里**没有** `## 22.`；A4 就是要加 `## 22.` | T-68 的一格 DoD。**已确认真的红了**，不是预测 |
...
**其余五条 `CRD 0019` 里没有**：四条是我拆任务时数出来的，**一条（`case-07`）我漏了，是 `crew-qa-1` 从 T-63 的真实红色里报回来的**——其中 `case-09` 那一条最容易被漏掉：
...
**本表第一版是 5 条，真实是 7 条，而漏掉的那一条是被第一个任务弄红的。** 这件事本身是结论：**会变红的用例数不完**，所以规则不能是「先数清再动手」，只能是「谁弄红谁在同一个提交里叫 QA 改」——那条规则对一条没预料到的红同样管用（`ADR 0018`）。

**这里有一件必须报给 PM 的事**：A1c 说「QA 只跑一轮，在编码结束之后」。但上面每一处必须在
