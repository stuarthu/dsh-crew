...
> QA 的用例会留在磁盘上并且能反复运行，范围或契约的每一次变更都有一份变更请求文档；
...
6. 它创建 `crew/<任务名>` 分支，每个任务启动一个 `crew_engineer`，一次只做一个里程碑。
...
9. **里程碑评审——PM 会停下来问你。** 当这个里程碑里的所有任务都过了上面那几道关
