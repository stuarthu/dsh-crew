...
- 任何点名审批文件的命令——连被信任的根 agent 也不能写，所以 agent 无法给自己授权。
  这个名字是按"整个文件名"来匹配的，所以只是包含它的更长的名字不会被牵连：
  `crew/push-ok-flow` 分支、`push-okay.md` 文件、`push-ok.bak` 备份，都不会被当成
  审批文件。
...
绕过，由 shell 拼出来的文件名也一样绕得过。反过来也有代价——只要命令里*提到*审批
文件的名字，就会被拒绝，你自己的会话也一样，所以提交信息里带 `push-ok` 的命令跑不
起来。真正的关口仍然是 dsh 自己的审批弹窗。
