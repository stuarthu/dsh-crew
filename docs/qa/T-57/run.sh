#!/usr/bin/env bash
# Runs every QA case of T-57. Exits 0 only when all of them pass.
#
# T-57 is a documentation-only task: it changes a prompt or a reader-facing file
...
echo "T-57: $cases case(s), $((cases - failed)) passed, $failed failed"
[ "$failed" -eq 0 ]
