...
5. 它决定写哪种文档并写出来：小活写 **DoD**（definition of done，"什么算做完"，
   `docs/crew/dod.md`），真正的产品写 **PRD**（product requirements document，
   产品需求文档，`docs/crew/prd.md`）。它会说明选了哪种，一个词就能换。
   **开工前必须你确认。**
...
8. 每个完成的任务按顺序过三道关：**代码评审**（先正确性，再看驱动这次改动的测试，
   ...
   **安全评审**（仅当改动涉及网络、登录鉴权、密钥、项目外的文件、shell、用户输入、
   ...
   用例都写成真实的测试文件，放在 `docs/crew/qa/<task-id>/`，用你项目自己的测试框架，
   旁边配一个 `run.sh`。`bash docs/crew/qa/run-all.sh` 会跑所有任务的用例，而且 QA 每次
...
11. **决定要发布的里程碑，会有两份计划；它们长什么样是查出来的，不是猜的。**
   ...
   - `docs/crew/release/<milestone>-release.md`——版本号和定它的规则、给用户看的发布
     说明、按顺序的确切步骤和每一步谁批准、开始之前必须成立的前提、事后怎么确认真的
     成功了，以及怎么撤回。如果撤不回来，计划里就直接这么写。
   - `docs/crew/release/<milestone>-upgrade.md`——谁在从哪些版本升上来、每一处破坏性
...
16. **修 bug 时可能会来问你，而且每一次选择都会被写下来。** 这件事可能在第 8 步里
    ...
    ADR——放在 `docs/crew/adr/` 的决策记录——由架构师来写；小活写进 `docs/crew/dod.md`
    里一个 **Decisions** 小节。两种写法内容一样：这个 bug 的病因、每一个选项及其代价、
...
`docs/crew/crd/NNNN-<short-name>.md`：谁提的、想要什么、为什么、会动到哪些文档和任务、
...
`qa/`）。作业状态放在仓库外的
`~/.dsh/crew/jobs/<job>/state.json`，这样 `git status` 保持干净。
