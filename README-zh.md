...
8. 每个完成的任务按顺序过三道关：**代码评审**（先正确性，再看驱动这次改动的测试，
   ...
   **安全评审**（仅当改动涉及网络、登录鉴权、密钥、项目外的文件、shell、用户输入、
   ...
   旁边配一个 `run.sh`。`bash docs/qa/run-all.sh` 会跑所有任务的用例，而且 QA 每次
...
16. **修 bug 时可能会来问你，而且每一次选择都会被写下来。** 这件事可能在第 8 步里
    ...
    决定会先写下来，之后才允许开工，而且里面要有**全部选项**。PRD 类的作业写成一条
    ADR——放在 `docs/decisions/adr/` 的决策记录——由架构师来写；小活写进
    `~/.dsh/crew/jobs/<job-slug>/dod.md` 里一个 **Decisions** 小节。两种写法内容一样：
    这个 bug 的病因、每一个选项及其代价、
    以后会痛在哪里、**为什么它输了**、选中了哪一个、是谁定的，以及理由。**每条 ADR
...
文档放在仓库里的 `docs/`，每个目录的名字就说清它装的是什么：PRD 和设计在 `docs/design/`
（每条模块边界一个契约文件，在 `docs/design/api/`）、决策记录和变更请求在
`docs/decisions/`（`adr/` 和 `crd/`）、QA 的测试计划和可重复运行的用例在 `docs/qa/`、
每个要发布的里程碑的发布计划和升级计划在 `docs/release/`、研究员的答案在 `docs/research/`。
DoD 和作业状态放在仓库外的 `~/.dsh/crew/jobs/<job-slug>/`（`dod.md` 和 `state.json`），
这样 `git status` 保持干净。
